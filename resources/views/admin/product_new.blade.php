@extends('admin.master', ['title' => 'Thêm sản phẩm']) @section('content')
<div class="container">
	<div class="row">
		<h1 class="title">
			<span>
		...
		<div class="col-md-8">
			<div class="form-group">
				<form class="form" method="post" id="new-product">
                    @if ($errors->any())
					...
					<textarea id="sort_desc" name="sort_desc">{{old('sort_desc')}}</textarea>
                    ...
                    <div class="input-group">
                        <input name="sale" value="{{old('sale') ?? 0 }}" id="sale"><label>%</label>
                    </div>
					<label for="keyword">Từ khoá (cách nhau bởi dấu , ):</label>
                    <input type="text" name="keyword" id="keyword" value="{{old('keyword')}}">
                    <div class="input-group">
                        <label>Sản phẩm HOT: </label><input type="checkbox" name="top" id="top" value="1">
                    </div>
                    <button class="btn-red">Thêm sản phẩm</button>
				</form>
			</div>
		</div>
		<div class="col-md-4">
